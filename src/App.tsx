...
// --- 动态生成照片列表 (top.jpg + 1.jpg 到 37.jpg) ---
const TOTAL_NUMBERED_PHOTOS = 37;
...
export default function GrandTreeApp() {
  ...
  // 🔊 BGM 相关：音频引用 + 是否需要用户点击
  const bgmRef = useRef<HTMLAudioElement | null>(null);
  const [needsInteraction, setNeedsInteraction] = useState(false);

  // 页面加载时：创建 Audio 并尝试自动播放
  useEffect(() => {
  ...
  // 如果自动播放被拦截：等待用户第一次点击任何地方再启动 BGM
  useEffect(() => {
    ...
    const onFirstInteraction = () => {
      if (bgmRef.current) {
        bgmRef.current
          ...
          .catch((err) => {
            console.error("Still cannot play BGM:", err);
          });
      }
